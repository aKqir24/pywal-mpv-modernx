...
local function load_colors()
    ...
    if f then
        ...
        -- Fallback colors (Default Dark/Material style)
        local defaults = {
...
-- OPTIMIZED: Use table buffers instead of string concat
...
function get_playlist()
    ...
    for i, v in ipairs(limlist) do
        ...
        if title == nil then title = filename end
...
function osc_init()
    ...
    ne.content = function ()
        if mp.get_property('pause') == 'no' then return ('\xEF\x8E\xA7')
        else return ('\xEF\x8E\xAA') end
    end
    ...
    ne.content = function ()
        if (state.mute) then return ('\xEF\x8E\xBB')
        else return ('\xEF\x8E\xBC') end
    end
    ...
    ne.content = function ()
        if (state.fullscreen) then return ('\xEF\x85\xAC')
        else return ('\xEF\x85\xAD') end
    end
    ne.visible = (osc_param.playresx >= 540)
    ...
    ne.slider.tooltipF = function (pos)
        ...
        if not ((duration == nil) or (pos == nil)) then
			...
			if #chapters > 0 then
				...
				if ch == 0 then
					return s_format('[%s] [0/%d]', mp.format_time(possec), #chapters)
				elseif chapters[ch].title then 
					return s_format('[%s] [%d/%d][%s]', mp.format_time(possec), ch, #chapters, chapters[ch].title)
				end
			end
    ...
    ne.slider.seekRangesF = function()
        if not user_opts.seekrange then return nil end
    ...
    ne.slider.tooltipF = function (pos)
			...
			if pos+3 >= refpos and pos-3 <= refpos then return s_format('%d', state.proc_volume)
			else return '' end
		end
    ne.slider.markerF = nil
    ne.slider.seekRangesF = nil
    ne.slider.posF = function () return state.proc_volume end
    ne.eventresponder['mouse_move'] = function (element)
            if not element.state.mbtnleft then return end
            local seekto = get_slider_value(element)
            if (element.state.lastseek == nil) or (not (element.state.lastseek == seekto)) then
    ...
    ne.eventresponder['mbtn_left_up'] = function (element) element.state.mbtnleft = false end
    ne.eventresponder['reset'] = function (element) element.state.lastseek = nil end
    ne.eventresponder['wheel_up_press'] = function (element) set_volume(state.proc_volume+5) end
    ne.eventresponder['wheel_down_press'] = function (element) set_volume(state.proc_volume-5) end
    
    -- tc_left (current pos)
    ne = new_element('tc_left', 'button')
    ne.content = function ()
        if (state.tc_ms) then return (mp.get_property_osd('playback-time/full'))
        else return (mp.get_property_osd('playback-time')) end
    end
    ne.eventresponder['mbtn_left_up'] = function ()
    ...
    ne.content = function ()
        ...
        if (state.rightTC_trem) then
            if (state.tc_ms) then return ('-'..mp.get_property_osd('playtime-remaining/full'))
            else return ('-'..mp.get_property_osd('playtime-remaining')) end
        else
            if (state.tc_ms) then return (mp.get_property_osd('duration/full'))
            else return (mp.get_property_osd('duration')) end
        end
    end
...
function hide_osc()
    ...
    else osc_visible(false) end
end

function osc_visible(visible)
    if state.osc_visible ~= visible then state.osc_visible = visible end
    request_tick()
end
...
function pause_state(name, enabled)
    ...
    if user_opts.showtitle then
        if enabled then
            ...
            show_osc()
        else visibility_mode(state.lastvisibility, true) end
    end
    request_tick()
end
...
function render()
    ...
    if not(state.anitype == nil) then
        if (state.anistart == nil) then state.anistart = now end
    ...
    else set_virt_mouse_area(0, 0, 0, 0, 'showhide_wc') end
    ...
    if osc_param.areas['window-controls'] then
        for _,cords in ipairs(osc_param.areas['window-controls']) do
            ...
            else mp.disable_key_bindings('window-controls') end
        ...
        if (user_opts.minmousemove == 0) or
            (not ((state.last_mouseX == nil) or (state.last_mouseY == nil)) and
                ((m_abs(mouseX - state.last_mouseX) >= user_opts.minmousemove)
                    or (m_abs(mouseY - state.last_mouseY) >= user_opts.minmousemove)
                )
            ) then
            show_osc()
        end
...
mp.observe_property('mute', 'bool', function(name, val) state.mute = val end)
mp.observe_property('volume', 'number', function(name, val)
		state.sys_volume = val
		if user_opts.processvolume then state.proc_volume = val*val/100
		else state.proc_volume = val end
	end)
...
mp.set_key_bindings({
    ...
    {'mbtn_mid', function(e) process_event('mbtn_mid', 'up') end, function(e) process_event('mbtn_mid', 'down') end},
